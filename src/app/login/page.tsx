...
// --- 수정된 부분: GoogleIcon에 viewBox 추가 ---
const GoogleIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg 
    xmlns="http://www.w3.org/2000/svg" 
    viewBox="0 0 24 24" // viewBox 추가
    {...props}
    >
...
const KakaoIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg 
    ...
    {...props}
    >
    <path d="M12 2c-5.523 0-10 3.582-10 8 0 2.934 1.973 5.515 4.811 6.845-.112.42-.397 1.518-.475 1.838-.135.563.093.57.25.467.157-.103 1.132-.767 1.65-1.11C9.28 18.82 10.6 19 12 19c5.523 0 10-3.582 10-8s-4.477-8-10-8z" fill="currentColor"/>
  </svg>
);
...
export default function LoginPage() {
  ...
  const handleSignInWithOAuth = async (provider: 'google' | 'kakao') => {
    setMessage('');
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gray-900">
      <div className="w-full max-w-sm p-8 space-y-6 bg-gray-800 rounded-lg shadow-xl">
        <h1 className="text-2xl font-bold text-center text-white">포트폴리오 로그인</h1>

        <div className="flex flex-col space-y-2">
          <button
            onClick={() => handleSignInWithOAuth('google')}
            className="flex items-center justify-center w-full px-4 py-2 font-medium text-gray-700 bg-white rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors"
          >
            {/* --- 수정된 부분: flex-shrink-0 추가 --- */}
          ...
          >
            {/* --- 수정된 부분: flex-shrink-0 추가 --- */}
            <KakaoIcon className="w-5 h-5 mr-2 flex-shrink-0" />
            <span>카카오로 로그인</span>
          </button>
        </div>
        ...
        <form onSubmit={handleSignIn} className="space-y-4">
          ...
          <div className="flex flex-col space-y-2 pt-2">
            ...
            <button
              ...
              className="w-full px-4 py-2 font-semibold text-indigo-300 bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors"
            >
              이메일로 회원가입
            </button>
          </div>
